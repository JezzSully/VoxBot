...
const raidDescribe = async (message, args) => {
  LOGGER.info('raid desribe request');
  const group = await getIndexForAuthor(message);
  if (!group) {
    LOGGER.info({ message: 'No raids owned by this author', author: message.author.id });
    return;
  }
  const oldEmbed = group.message;
  args.splice(0, 1);
